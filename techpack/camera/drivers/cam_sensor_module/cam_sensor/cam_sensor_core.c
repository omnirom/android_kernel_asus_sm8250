	...
	void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		if (cmd->handle_type ==
			...
			if (rc < 0) {
				CAM_ERR(CAM_SENSOR, "Get Buffer Handle Failed sensor_id = 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
				goto release_mutex;
			}
		} else {
			CAM_ERR(CAM_SENSOR, "Invalid Command Type: %d sensor_id = 0x%x",
				 cmd->handle_type,s_ctrl->sensordata->slave_info.sensor_id);
			rc = -EINVAL;
			goto release_mutex;
		}
		CAM_INFO(CAM_SENSOR,
				"Probe sensor_id = 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
		...
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
				"Fail in filling vreg params for PUP sensor_id = 0x%x rc %d",
		...
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
				"Fail in filling vreg params for PDOWN sensor_id = 0x%x rc %d",
				 s_ctrl->sensordata->slave_info.sensor_id,rc);
			goto free_power_settings;
		}
		...
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "power up failed sensor_id = 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			goto free_power_settings;
		}
		...
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "match id failed sensor_id = 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			cam_sensor_power_down(s_ctrl);
			msleep(20);
			goto free_power_settings;
		}
		...
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR, "fail in Sensor Power Down sensor_id = 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
			goto free_power_settings;
		}
		/*
		...
		s_ctrl->sensor_state = CAM_SENSOR_INIT;
		CAM_INFO(CAM_SENSOR,
				"Probe X sensor_id = 0x%x",s_ctrl->sensordata->slave_info.sensor_id);
